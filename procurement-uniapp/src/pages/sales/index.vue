<template>
  <view class="page-sales">
    ...
    <view class="order-list container">
      ...
      <EmptyState v-if="orderList.length === 0 && !loading" text="暂无销售订单" buttonText="立即开单" @action="goCreateOrder" />
      <LoadMore v-if="orderList.length > 0" :status="loadMoreStatus" @load="loadMore" />
    </view>

    <!-- 底部操作 -->
    <view class="bottom-bar safe-area-bottom">
      <view class="bottom-btn" @tap="goCustomerList">
        <text class="bottom-btn__text">客户</text>
      </view>
      <view class="bottom-btn bottom-btn--primary" @tap="goCreateOrder">
        <text class="bottom-btn__text bottom-btn__text--white">+ 开单</text>
      </view>
    </view>
  </view>
</template>
...
<script>
import { getSalesOrderList } from '@/api/salesOrder'
...
.status-tab {
  ...
  &__text { font-size: 26rpx; color: #666; }
}

.order-card {
  &__header { margin-bottom: 12rpx; }
  &__no { font-size: 28rpx; font-weight: 600; color: #333; }
  &__customer { font-size: 26rpx; color: #666; margin-bottom: 12rpx; }
  &__label { color: #999; }
  &__amount { font-size: 32rpx; }
  &__date { font-size: 24rpx; color: #999; }
}
...
.bottom-btn {
  ...
  &--primary { background: #2979ff; }
  &__text { font-size: 28rpx; color: #666; &--white { color: #fff; } }
}
</style>
